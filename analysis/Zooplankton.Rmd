---
title: "Zooplankton de Tulear à Madacascar"
output:
  html_document:
    highlight: pygments
    theme: cosmo
source: Grosjean, Ph & K. Denis (2004). Supervised classification of images, applied to plankton samples using R and ZooImage. Chap.12 of Data Mining Applications with R. Zhao, Y. & Y. Cen (eds). Elevier. Pp 331-365. https://doi.org/10.1016/C2012-0-00333-X.
---

```{r setup, echo = FALSE,include= FALSE}
SciViews::R
```
#Introduction
Nous avons récolté des échantillons de zooplanctons en différents point géographiques, en différentes saisons, en différentes profondeurs pour pouvoir représenter la variabilité. Attention les classes rares sont ici surreprésentées.

#But
Le but de cette expérience est de paramétrer un classeur automatique pour le zooplancton. Pour cela, on 



#Graphique
```{r import}
zooplankton <- read( file = "zooplankton", package = "data.io", lang = "fr")
```

```{r}
zooplankton %>.%
  group_by(., class) %>.%
  summarise(., "ecd"=mean(ecd),
            "area"= mean(area),
            "perimeter"= mean(perimeter),
            "feret"=mean(feret),
            "major"=mean(major),
            "minor"=mean(minor),
            "mean"=mean(mean),
            "mode"=mean(mode),
            "min"=mean(min),
            "max"=mean(max),
            "std_dev"=mean(std_dev),
            "range"=mean(range),
            "size"=mean(size),
            "aspect"=mean(aspect),
            "elongation"=mean(elongation),
            "compactness"=mean(compactness),
            "transparency"=mean(transparency),
            "circularity"=mean(circularity),
            "density"=mean(density)) -> zoo
```
##Graphique de l'aire en fonction du diamètre circulaire équivalent.
```{r g1}
chart(zoo, area~ecd %col=% class) +
  geom_point(na.rm = TRUE) +
  scale_fill_viridis_d()
```

##Histogramme de la taille. 
```{r g2}
chart(zoo, formula = ~ size %fill=% class) +
  geom_histogram(bins= 30) +
  geom_histogram(show.legend = FALSE)
```
```{r}
chart(zoo,size~class) +
  geom_col(position = "dodge") +
  coord_flip()
```

```{r g5_6}
chart(zoo, formula = ~ size %col=% class) +
  geom_density()
chart(zoo, ~size %color=% class) +
  geom_density()
```

##Graphique de l'aire pour chaque classe de zooplancton.

```{r ,echo= FALSE}
library(ggridges)
```

```{r g3}
chart(zoo, class ~ area ) +
  geom_density_ridges(na.rm = TRUE)
```
```{r g9}
chart(zooplankton, area~class) +
  geom_violin(na.rm = TRUE) +
  coord_flip()
```

##Graphique de la transparence en fonction de la compacité.
```{r g4}
chart(zooplankton, transparency~compactness |class) +
  geom_point(na.rm = TRUE)
```
```{r}
library(ggplot2)
```
```{r}
ggplot(zooplankton, aes(compactness=x, transparency=y)) +
  geom_segment( aes(x=x, xend=x, y=0, yend=y), size=1, color="blue", linetype="dotted") +
  geom_point()
```

##Graphique de la taille pour chaque classe.
```{r g7}
chart(zooplankton, formula = size ~ class) +
  geom_violin() +
  coord_flip()
```

##Graphique de l'aire en fonction de l'axe majeur de l'éllipsoïde.
```{r g8}
chart(zooplankton, area~major %color=% class) +
  geom_point(na.rm = TRUE)
```


##Graphique de l'étendue des D.O. en fonction de la taille .
```{r g10}
chart(zooplankton, range~size |class) +
  geom_point(na.rm = TRUE)
```

##Graphique des D.O. minimales en fonction des D.O. maximums
```{r g11}
chart(zooplankton, min~max |class) +
  geom_point(na.rm =TRUE)
```

##Graphique de la densité en fonction des D.O. minimale
```{r g12}
chart(zooplankton, ~min) +
  geom_density(na.rm = TRUE)
```
 
##Histograme de l'écartype des D.O.
```{r g13}
chart(zooplankton,~std_dev |class) +
  geom_histogram(data = zooplankton[ , -c(20)], fill = "light blue") +
  geom_histogram(show.legend = FALSE)
```

##Graphique de la circularité en fonction du périmètre.
```{r g14}
chart(zooplankton, circularity~perimeter |class) +
  geom_point()
```
```{r}
chart(zooplankton,circularity~perimeter) +
  geom_bar(position = "dodge") +
  coord_flip()
```

##Histograme des aspects.
```{r g15}
chart(zooplankton,~aspect |class) +
  geom_histogram(data = zooplankton[,-c(20)], fill = "light blue") +
  geom_histogram(show.legend = FALSE)
```
```{r}
chart(zoo_peri, aes(x=mean, y=class))+
  geom_point() +
  geom_segment(aes(x=0,
                   xend=mean,
                   y=class,
                   yend=class))
``` 

```{r nouveau jeu de données}
zooplankton %>.%
  group_by(., class) %>.%
  summarise(.,mean= mean(perimeter)) -> zoo_peri
```
